          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 md:gap-12 items-center">
            <ScrollAnimation direction="left">
              ...
              <StaggerContainer className="space-y-3 md:space-y-4 mb-8" staggerDelay={0.1}>
                {[
                  ...
                  'Maintain consistent temperatures for employees and inventory',
                  'Minimize equipment downtime and maximize productivity'
                ].map((item, index) => (
            ...
            <ScrollAnimation direction="right" delay={0.2}>
              <motion.div whileHover={{ scale: 1.02 }} transition={{ duration: 0.3 }}>
                <Image src="/images/warehouse-insulation.jpg" alt="Commercial insulation project" width={600} height={450} className="w-full h-auto rounded-lg shadow-xl" />
              </motion.div>
            </ScrollAnimation>
          </div>
        </div>
      ...
      <section className="py-16 md:py-20 bg-purple-dark overflow-hidden">
        <div className="container-custom">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 md:gap-12">
            <ScrollAnimation direction="left">
              <h2 className="text-2xl md:text-3xl font-bold text-white uppercase mb-4">Get Your Free Commercial Quote</h2>
            ...
            <ScrollAnimation direction="right" delay={0.2} className="flex flex-col justify-center">
              ...
              <div className="grid grid-cols-2 gap-2">
                {cities.slice(0, 20).map((city) => (
                  <Link key={city.slug} href={`/commercial-insulation-${city.slug}-tx/`} className="text-gray-300 hover:text-tan-gold text-xs md:text-sm transition-colors">
                    {city.name}, TX
                  </Link>
                ))}
              </div>
